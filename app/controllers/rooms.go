...
/***
* start chat with with user
 */
func OpenRoomAdmin(g *gin.Context) {
	...
	if oldRoom.ID == 0 {
		helpers.ReturnNotFound(g, "Not found user room!")
		return
	}
...
/***
* start chat with with user and  create new room if this first time chat to admin
 */
func OpenRoomNormal(g *gin.Context) {
	// get user from header
	user1 := models.GetUserFromHeader(g)
	// get user how you want to chat with him
	user_id, _ := strconv.Atoi(g.Param("id"))
